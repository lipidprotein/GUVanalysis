...
%% 8. Phase decision (0-uniform, 1- binary pahse)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
trackDist = 10;%5; %pixel distance to consider the same vesicles
...
PathName = 'Sample';
filelistA = dir([PathName, '/A/*.tif']);
filelistB = dir([PathName, '/B/*.tif']);
...
%[neuralFile neuralPath] = uigetfile('*.mat','Choose the neural network for image filtering to load.');
%load([neuralPath, neuralFile]);
load('./nrFilter4C.mat');
%[neuralFile2 neuralPath2] = uigetfile('*.mat','Choose the neural network file to state decision to load.');
%load([neuralPath2, neuralFile2]);
load('./nrFilter2Cstate_net2');
%load('./net2_created');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for index=1:1:length(filelistA)
  ...
  for i=1:1:size(intensity,1)
    ...
    for ix=round(centers(i,1)-r15):1: round(centers(i,1)+r15)
      for iy=round(centers(i,2)-r15):1: round(centers(i,2)+r15)
        if (ix<= 0) || (iy<=0) ||(ix>size(imgB,2))||(iy>size(imgB,1))
         continue
        end
        ...
        %%if (abs(radii(i) - dist) < thres)
          intensity(i,1) = intensity(i,1) + int32(imgB(iy, ix));    
    ...
    for seg = 1:1:num_seg
      if seg_pixels(seg) == 0
        ...
        intensity(i, 3+2+seg) = intensity(i, 3+2+seg) - backgroundIntB;
      end
    end 
    intensity(i,3) = intensity(i,1) / intensity(i,2);
    intensity(i,4) = backgroundIntB;
    intensity(i,5) = intensity(i,3) - backgroundIntB;
  end
  ...
  for i=1:1:size(intensity,1)  
    ...
    %%%%%%%%%%%%%%%%%%%%%%%
    %% final phase decision (modified for CNN analysis from the original intensity analysis)
    ...
    nrMargin = 1.2; % to multiply to make sure image contains the circle.
        ...
        nrImage = imgB(nrY1:nrY2,nrX1:nrX2);
        

        nrImage = imresize(nrImage,[50, 50]);
     
        %% deNosing block
        %[lehisto x] = imhist(nrImage,11107); %triangle thresholding 
        %maskThres = 65535 * triangle_th(lehisto, 11107);
        
        maskThres = mode(nrImage(:)); %moment thresholding 
        nrImage = nrImage - maskThres;     
        nrImage(nrImage < 0) = 0;
        %%   
        
        [decision, probNR] = classify(net2,nrImage);
        if (decision == 'C1')
            phase = 2;
        else
            phase = 0;
...
elseif isCloseStackfilt == 2
  ...
  for indexGUV = 1:1:length(GUV)
    ...
    if sortGUV(round((1-decisionP)*length(sortGUV))) >= 2
      phaseGUV = 2; %multi phase (not applicable inthis case) 
  ...
  disp(['Total ', num2str(length(GUV)),' GUVs. Uniform: ', num2str(100*countUniform/length(GUV)),'%, Phase: ', num2str(100*(countPhase+countMulti)/length(GUV)), '%']);
  disp(' ');
  ...
  fid = fopen([PathOut '/' OutName], 'wt');
  fprintf(fid, '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n','X_avg_coordinate', 'Y_avg_coordinate', 'max_radii', 'num_slices', 'avg_intensity', 'max_intensity', 'min_intensity', 'Phase_0uniform_1separated' );  % header
  fclose(fid);
  ...
  if size(imgAinfo,1) >4
   ...
   imgB = imread(fnameB,1);
  end

  figure(size(imgAinfo,1) + 1);
  imshow(imgB, [min(min(imgB)) 0.5*max(max(imgB))]);
  title('ChB overlapped with final GUVs detected. Cyan-uniform, Magenta-binary');
